...
- Extend the authoring/import workflow toward galgame source intake, script dialogue extraction, persona-card generation, memory candidates, visual mapping, voice mapping, and demo-world assembly.
...
- Future backend work will likely touch provider settings, provider adapters, model discovery, authoring, media, visual, speech, memory, conversations, and readiness/eval packages.
- Future Web work will likely touch provider settings/model lab, import review, asset mapping, voice mapping, and demo-world setup surfaces; those surfaces must use the existing Noveland product UI style and avoid marketing/gamey admin patterns.
