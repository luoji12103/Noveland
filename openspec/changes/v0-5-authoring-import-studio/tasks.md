# Tasks — v0.5 Authoring & Import Studio

Use these tasks when implementation is explicitly requested. Do not mark items complete during roadmap-only work.

## Phase 1 — Authoring Import Core

- [x] Write phase planning checkpoint.
- [x] Implement dedicated `backend/packages/authoring/` package and `authoring.py` router boundary.
- [x] Implement `authoring-source-registry` and `import-preview-apply-workflow` foundation scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 2 — Script Parser & Dialogue Extractor

- [x] Write phase planning checkpoint.
- [x] Implement `script-parser-dialogue-extractor` scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 3 — Character & Relationship Extractor

- [x] Write phase planning checkpoint.
- [x] Implement `character-relationship-extractor` scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 4 — World Bible & Lore Extractor

- [x] Write phase planning checkpoint.
- [x] Implement `world-bible-lore-extractor` proposal-only scope.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 5 — Canon Conflict Review

- [x] Write phase planning checkpoint.
- [x] Implement `canon-conflict-review` scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 6 — Memory Migration Pipeline

- [x] Write phase planning checkpoint.
- [x] Implement `memory-migration-pipeline` scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 7 — Asset Import & Matching

- [x] Write phase planning checkpoint.
- [x] Implement `asset-import-matching` scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [x] Fast-forward merge to local main.
- [x] Update OpenSpec tasks and harness docs.

## Phase 8 — Authoring Regression Fixture

- [x] Write phase planning checkpoint.
- [x] Implement `authoring-regression-fixture` scope only.
- [x] Preserve Phase 13 architecture guardrails.
- [x] Add or update focused tests.
- [x] Run targeted tests.
- [x] Run full local gate.
- [ ] Fast-forward merge to local main.
- [ ] Update OpenSpec tasks and harness docs.
