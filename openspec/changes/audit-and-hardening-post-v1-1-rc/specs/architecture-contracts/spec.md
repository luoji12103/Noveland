...
- **WHEN** the `worldline_id` is missing from the requested `world_id` or belongs to another world
- **THEN** the API SHALL reject the request with a handled client error instead of surfacing an unhandled service exception
- **AND** valid worldline-scoped speech list operations SHALL continue to return voice profiles, agent bindings, or transcripts from the resolved worldline scope.

#### Scenario: Visual generation model asset lists reject invalid worldline scope consistently
- **GIVEN** a visual generation model asset list request includes an explicit `worldline_id`
- **WHEN** the `worldline_id` is missing from the requested `world_id` or belongs to another world
...
#### Scenario: Member reads sensitive-looking agent calendar text
- **GIVEN** a world member lists agent calendar entries through member-readable routes
- **WHEN** calendar title, description, or recurrence text contains storage refs, filesystem paths, raw prompt/output markers, secret/auth refs, bytes, or base64
...
#### Scenario: Member reads sensitive-looking worldline and agent identity text
- **GIVEN** a world member lists worldlines, agents, or agent relationships through member-readable routes
- **WHEN** worldline names/descriptions, agent display names, or relationship source/target display names contain storage refs, filesystem paths, raw prompt/output markers, secret/auth refs, bytes, or base64
- **THEN** the member response SHALL blank the sensitive-looking text while preserving safe worldline names/descriptions, safe agent display names, identity keys, relationship state, metadata redaction, and existing admin-field redaction
- **AND** admin routes MAY continue to expose full worldline and agent identity text required for world management and runtime planning.
