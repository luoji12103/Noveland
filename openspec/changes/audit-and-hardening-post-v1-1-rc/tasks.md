...
### F-125 Conversation realtime member deltas leak sensitive turn transcript text

- Severity: High
- Affected boundary: Backend realtime conversation stream and live snapshot member-safe turn DTOs.
- Evidence: Temporary CLI reproduction using existing `backend/tests/test_api_realtime.py` in-memory fixtures showed `collect_conversation_stream_delta(..., include_admin_fields=False)` returns turn `input_text` and `output_text` containing `raw_output: provider traceback payload`, while the REST conversation turn response already blanks sensitive-looking member transcript text.
- Impact: ordinary world members receiving realtime conversation deltas or live WebSocket snapshots can see raw prompt/output markers, provider traces, storage refs, secrets, or other operator-only execution evidence embedded in turn transcript text, weakening the reader/member boundary during live playback.
...
### F-127 Reader media delivery serves active content MIME types inline

- Severity: High
- Affected boundary: Reader/member media delivery object download and descriptor content-type safety.
- Evidence: Temporary CLI reproduction using existing `backend/tests/test_api_reader_media.py` in-memory fixtures showed a `reader_visible` `video` asset referenced from a published artifact can store an object with `mime_type="text/html"`; `GET /worlds/{world_id}/reader/media` returns the descriptor with `content_type=text/html`, and the scoped reader download route returns `200 text/html; charset=utf-8` with attacker-controlled HTML bytes despite `X-Content-Type-Options: nosniff`.
- Impact: a world member or reader could navigate to a same-origin reader media download URL serving active HTML/script content under the application origin, creating an XSS/session-boundary risk and violating reader media delivery safety expectations.
- Intended remediation: restrict reader media descriptors and downloads to safe MIME types for reader-deliverable image/audio/video kinds, omit unsafe objects from descriptors, hide assets that have no safe objects, preserve safe image/audio/video delivery, and add focused reader media regression coverage.
- Status: Remediated in reader media active content-type suppression batch.
...
### F-128 Player resume marks media without safe reader objects as ready

- Severity: Medium
- Affected boundary: Backend player session recovery, reader media availability, and player-safe playback state.
- Evidence: Temporary CLI reproduction using existing `backend/tests/test_api_player_sessions.py` fixtures showed a presentation with an `available`/`player_visible` composite media asset but no reader-deliverable media object returns `recovery_status=ready` and includes `open_reader_playback`, even though reader media delivery cannot serve a safe object for that asset. After F-127, the same mismatch also applies to assets whose objects are filtered for active content MIME types.
- Impact: player resume can advertise playback as ready when the referenced presentation media will be absent or unsafe at reader delivery time, creating inconsistent player recovery UX and weakening the player/media safety boundary.
- Intended remediation: require presentation media assets to have at least one safe reader-deliverable media object before player resume treats them as ready, preserve existing missing/failed/private/cross-worldline behavior, and add focused regression coverage.
- Status: Remediated in player resume safe media object batch.
- Verification: Temporary CLI reproduction first showed objectless player-visible presentation media returning `recovery_status=ready` with `open_reader_playback`; `cd backend && uv run pytest tests/test_api_player_sessions.py::test_player_session_media_without_safe_reader_objects_is_missing_media -q` first failed before remediation, then passed after requiring a safe media object; `cd backend && uv run pytest tests/test_api_player_sessions.py -q` passed with 5 tests; focused `cd backend && uv run ruff check packages/player_sessions/src/noveland/player_sessions/service.py tests/test_api_player_sessions.py` and matching mypy command passed; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 583 tests and 8 skipped.

### F-129 Player resume ignores reader descriptor reference requirements

- Severity: Medium
- Affected boundary: Backend player session recovery, reader media descriptor availability, and player-safe playback state.
- Evidence: Temporary CLI reproduction using existing `backend/tests/test_api_player_sessions.py` fixtures plus reader media dependencies showed a presentation with a `player_visible` image asset and safe `image/png` object but no `MediaReference` returns `recovery_status=ready` with `open_reader_playback`; the same session's `GET /worlds/{world_id}/reader/media?worldline_id=...` returns `[]`, and member presentation readback hides `composite_scene_asset_id` because `ReaderMediaDeliveryService.get_media()` has no descriptor.
- Impact: player resume can still advertise ready playback when reader playback cannot resolve a media descriptor, creating inconsistent resume UX and leaving player readiness weaker than the reader media delivery boundary.
- Intended remediation: align player resume media readiness with `ReaderMediaDeliveryService.get_media()` descriptor availability, preserving safe referenced media and returning missing-media for assets lacking reader-visible references, safe objects, or other descriptor requirements.
- Status: Remediated in player resume reader descriptor alignment batch.
- Verification: Temporary CLI reproduction first showed a safe-object/no-reference presentation returning `recovery_status=ready` with `open_reader_playback`, while reader media returned `[]` and member presentation hid the media asset id; `cd backend && uv run pytest tests/test_api_player_sessions.py::test_player_session_media_without_safe_reader_objects_is_missing_media -q` passed after remediation; `cd backend && uv run pytest tests/test_api_player_sessions.py -q` passed with 5 tests; focused `cd backend && uv run ruff check packages/player_sessions/src/noveland/player_sessions/service.py tests/test_api_player_sessions.py` and matching mypy command passed; related `cd backend && uv run pytest tests/test_api_player_sessions.py tests/test_api_reader_media.py tests/test_api_conversation_presentations.py -q` passed with 14 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 583 tests and 8 skipped.

### F-130 Web playback substitutes unrelated referenced media for missing explicit presentation media

- Severity: Medium
- Affected boundary: Web reader playback/scene media resolution, member-safe presentation media availability, and deterministic missing-media UX.
- Evidence: Read-only audit showed `web/features/worlds/playback-media.ts` resolves `presentation.composite_scene_asset_id`, `presentation.background_asset_id`, and `presentation.tts_media_asset_id` through the reader media descriptor map, but if an explicit presentation id is absent from descriptors it falls through to `preferredReferencedMedia()` over same-turn or same-session media. After backend safety filtering hides unavailable/unsafe/no-reference presentation asset ids from member presentation responses and reader media descriptors, the Web UI can render a different referenced image/audio instead of the missing-media state for the explicit presentation media.
- Impact: reader playback and scene view can show stale or unrelated media for a turn whose canonical presentation media is unavailable, weakening the reader media delivery boundary and making player resume/playback recovery state nondeterministic.
- Intended remediation: distinguish absent presentation ids from unresolved explicit ids in shared playback media resolution, block referenced fallback for a media kind when any explicit presentation id for that kind is present but unresolved, preserve referenced fallback only for turns with no explicit presentation id, and add focused Web regression coverage for both paths.
- Status: Remediated in Web playback explicit-media fallback batch.
- Verification: `cd web && npm run test -- features/worlds/playback-media.test.ts` first failed because unresolved explicit presentation media fell through to `referenced-image`, then passed after remediation with 3 tests; `cd web && npm run test -- features/worlds/playback-media.test.ts features/worlds/conversation-playback.test.tsx features/worlds/conversation-scene-view.test.tsx` passed with 3 files and 11 tests; `cd web && npm run typecheck -- --pretty false`, `cd web && npm run lint -- features/worlds/playback-media.ts features/worlds/playback-media.test.ts features/worlds/conversation-playback.test.tsx features/worlds/conversation-scene-view.test.tsx`, full `cd web && npm run test` with 53 files and 212 tests, `cd web && npm run build`, and `cd web && npm run check:next-env` passed. `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed. Web e2e was not run for this shared resolver-only batch.

### F-131 Reader media ignores moderation suppression on referenced surfaces

- Severity: High
- Affected boundary: Reader/member media delivery, content moderation takedown enforcement, and reference visibility.
- Evidence: Temporary CLI reproduction using existing reader media API fixtures created a reader-visible media asset referenced from a conversation turn, then inserted an applied `takedown_content` moderation action targeting that conversation turn with `reader_delivery_suppression=true`; `GET /worlds/{world_id}/reader/media`, detail, and scoped object download still returned the media with 200 responses. Code review showed `ReaderMediaDeliveryService._descriptor_for_asset()` only checks `MEDIA_ASSET` suppression, while moderation also supports suppression targets for worldlines, narrative publications, conversation sessions, and conversation turns.
- Impact: moderated conversation or publication surfaces can continue serving attached media to readers/members unless every media asset is individually suppressed, weakening takedown semantics and reader delivery visibility enforcement.
- Intended remediation: make reader media descriptor/reference resolution treat applied moderation suppression on worldlines and supported reference targets as reader-invisible, preserve delivery when an asset has another unsuppressed reader-visible reference, and add focused reader media API regression coverage for suppressed conversation-turn references.
- Status: Remediated in reader media reference moderation batch.
- Verification: Temporary CLI reproduction first showed a `conversation_turn` applied `takedown_content` moderation action with `reader_delivery_suppression=true` still allowed reader media list/detail/download responses with 200 statuses. `cd backend && uv run pytest tests/test_api_reader_media.py::test_reader_media_suppresses_moderated_reference_targets -q` first failed because moderated turn/session references still returned descriptors, then passed after remediation; `cd backend && uv run pytest tests/test_api_reader_media.py::test_reader_media_suppresses_moderated_reference_targets tests/test_api_reader_media.py::test_reader_media_suppresses_moderated_worldline_and_publication_targets tests/test_api_reader_media.py::test_reader_media_keeps_unsuppressed_references_when_one_reference_is_moderated -q` passed with 3 tests; `cd backend && uv run pytest tests/test_api_reader_media.py tests/test_api_moderation.py tests/test_api_player_sessions.py tests/test_api_conversation_presentations.py -q` passed with 24 tests; focused `cd backend && uv run ruff check packages/reader_delivery/src/noveland/reader_delivery/service.py tests/test_api_reader_media.py` and matching mypy command passed; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 586 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-132 Moderation accepts unresolved scene, publication, and player profile target refs

- Severity: High
- Affected boundary: moderation report/action target validation, world/worldline isolation, and reader/player takedown accuracy.
- Evidence: Read-only audit showed `backend/packages/moderation/src/noveland/moderation/service.py` marks `narrative_publication` and `player_profile` as worldline-scoped but `_validate_target()` only validates worldline/provider/conversation/media targets. `scene` targets also lack an ownership check despite having a concrete world-owned model. As a result, moderation mutations can persist arbitrary UUIDs for these target kinds after only validating the optional `worldline_id`.
- Impact: members/admins can create reports or applied actions against missing or cross-scope content/profile refs, producing misleading moderation audit records and weakening takedown/disable semantics for reader/player-visible surfaces that later rely on accurate moderation targets.
- Intended remediation: validate concrete scene, narrative publication, and player profile targets against their owning world, and require narrative publication/player profile refs to belong to the supplied worldline; preserve intentionally free-form `other` and currently non-persistent plugin package refs.
- Status: Remediated in moderation concrete target validation batch.
- Verification: `cd backend && uv run pytest tests/test_api_moderation.py::test_moderation_rejects_unresolved_concrete_target_refs -q` first failed because an unresolved `narrative_publication` target returned 201, then passed after remediation; focused `cd backend && uv run ruff check packages/moderation/src/noveland/moderation/service.py tests/test_api_moderation.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_moderation.py tests/test_api_reader_media.py tests/test_api_player_sessions.py tests/test_api_conversation_presentations.py -q` passed with 25 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-133 Member agent calendar text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable agent calendar REST API and admin-authored calendar text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/agents/{agent_id}/calendar` through `get_world_member_context` and calls `_calendar_entry_response(..., include_admin_fields=False)`. That helper redacts metadata for members but returns `title`, `description`, and `recurrence_rule` unchanged even though admin calendar create/update accepts arbitrary text.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in calendar entry text despite metadata being redacted.
- Intended remediation: blank sensitive-looking calendar title, description, and recurrence text for non-admin member responses while preserving safe calendar text and admin visibility; add focused regression coverage in the existing calendar API test.
- Status: Remediated in member calendar text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_world_admin_manages_calendar_entries_and_schedule_rules -q` first failed because member calendar title text containing `raw_prompt` rendered unchanged, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-134 Member scene and location graph text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable scene and location graph REST APIs and admin-authored location text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/scenes` and `GET /worlds/{world_id}/location-edges` through `get_world_member_context`. `_scene_response(..., include_admin_fields=False)` redacts `opening_rules` but returns `name` and `description` unchanged, while `_location_edge_response(..., include_admin_fields=False)` redacts `traversal_rules` but returns `travel_label` unchanged.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in scene or travel-label text despite rule/config redaction.
- Intended remediation: blank sensitive-looking scene name, scene description, and location travel label text for non-admin member responses while preserving safe text and admin visibility; add focused regression coverage in the existing scene/location API test.
- Status: Remediated in member scene/location text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_location_graph_and_agent_presence_enforce_world_scope -q` first failed because member scene name text containing `raw_prompt` rendered unchanged, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-135 Member organization and faction text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable organization, organization membership, and faction progress track REST APIs and admin-authored organization/faction text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/organizations`, `GET /worlds/{world_id}/organizations/{organization_id}/memberships`, and `GET /worlds/{world_id}/organizations/{organization_id}/faction-tracks` through `get_world_member_context`. `_organization_response(..., include_admin_fields=False)` redacts `hidden_summary` and metadata but returns `name`, `description`, and `public_summary` unchanged; `_organization_membership_response(..., include_admin_fields=False)` redacts metadata but returns `organization_name`, `role_title`, and `responsibilities` unchanged; `_faction_track_response(..., include_admin_fields=False)` redacts metadata but returns `organization_name`, `name`, and `summary` unchanged.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in organization, membership, or faction text despite metadata/hidden field redaction.
- Intended remediation: blank sensitive-looking organization, membership, and faction text for non-admin member responses while preserving safe public text, identity fields, numeric state, metadata redaction, and admin visibility; add focused regression coverage in the existing organization/faction API test.
- Status: Remediated in member organization/faction text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_organization_memberships_and_faction_tracks_append_events -q` first failed because member organization name text containing `raw_prompt` rendered unchanged, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-136 Member worldline and agent identity text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable worldline, agent list, and agent relationship REST APIs and admin-authored worldline/agent identity text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/worldlines`, `GET /worlds/{world_id}/agents`, and `GET /worlds/{world_id}/agents/{agent_id}/relationships` through `get_world_member_context`. `_worldline_response(..., include_admin_fields=False)` redacts metadata but returns `name` and `description` unchanged; `_agent_response(..., include_admin_fields=False)` redacts config/provider/admin fields and sanitizes `character_profile` but returns `display_name` unchanged; `_agent_relationship_response(..., include_admin_fields=False)` redacts metadata but returns source/target `display_name` values unchanged.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in worldline or agent identity text despite metadata/config redaction.
- Intended remediation: blank sensitive-looking worldline names/descriptions, agent display names, and relationship source/target display names for non-admin member responses while preserving safe text, identity keys, relationship state, metadata redaction, and admin visibility; add focused regression coverage in existing worldline, agent, and relationship API tests.
- Status: Remediated in member worldline/agent identity text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_agent_relationship_graph_enforces_world_scope_and_updates_edges tests/test_api_worlds.py::test_create_agent_from_preset_materializes_persona_calendar_and_provider_mapping tests/test_api_worlds.py::test_world_member_can_read_safe_worldline_comparison_without_mutation -q` first failed because member relationship display, member agent display, and member worldline name text containing sensitive markers rendered unchanged, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-137 Member schedule rule name text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable schedule rule REST API and admin-authored schedule rule text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/schedule-rules` through `get_world_member_context`. `_schedule_rule_response(..., include_admin_fields=False)` redacts `config` but returns `name` unchanged even though admin schedule rule create/update accepts arbitrary names.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in schedule rule names despite config redaction.
- Intended remediation: blank sensitive-looking schedule rule names for non-admin member responses while preserving safe names, rule keys, kind, enabled state, config redaction, and admin visibility; add focused regression coverage in the existing calendar/schedule API test.
- Status: Remediated in member schedule rule text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_world_admin_manages_calendar_entries_and_schedule_rules -q` first failed because member schedule rule name text containing `raw_prompt` rendered unchanged, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-138 Member player actor display text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable player actor REST APIs and player/admin-authored actor display text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/player-actors` and `PUT /worlds/{world_id}/player-actors` through `get_world_member_context`. `_player_actor_response()` sanitizes `profile` JSON but returns `display_name` unchanged, and the bind route accepts arbitrary display names from the acting member or an admin acting on behalf of another user.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in player actor display names despite profile redaction.
- Intended remediation: blank sensitive-looking player actor display names for non-admin member responses while preserving safe names, actor identity fields, current scene refs, profile redaction, and admin visibility; add focused regression coverage in the existing player interaction API test.
- Status: Remediated in member player actor text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_world_member_can_use_own_player_interaction_records_without_admin_scope -q` first failed because member player actor display text containing `raw_prompt` and a filesystem path rendered unchanged, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.

### F-139 Member world summary text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable world list/detail REST APIs and admin-authored world name/description text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds` and `GET /worlds/{world_id}` to ordinary world members. `_world_response(..., include_admin_fields=False)` redacts rules/plugin configuration but returns `name` and `description` unchanged even though platform/world admin create/update accepts arbitrary world summary text.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, or base64-like evidence embedded in world names or descriptions despite configuration redaction.
- Intended remediation: blank sensitive-looking world names/descriptions for non-admin member responses while preserving safe world summary text, slug, active state, admin-field redaction, and admin visibility; add focused regression coverage in the existing world membership read test.
- Status: Remediated in member world summary text redaction batch.
- Verification: `cd backend && uv run pytest tests/test_api_worlds.py::test_world_member_can_read_but_not_mutate_and_non_member_is_hidden -q` first failed because member world list/detail returned world name text containing `raw_prompt`, then passed after remediation; focused `cd backend && uv run ruff check services/api/src/noveland/services/api/worlds.py tests/test_api_worlds.py` and matching mypy command passed; `cd backend && uv run pytest tests/test_api_worlds.py -q` passed with 42 tests; full `cd backend && uv run ruff check .`, `cd backend && uv run mypy .`, and `cd backend && uv run pytest` passed with 587 tests and 8 skipped; `openspec validate audit-and-hardening-post-v1-1-rc --strict`, `openspec validate --changes --strict`, `openspec validate --specs --strict` with 76 specs, and `git diff --check` passed.


### F-140 Member conversation narrative artifact text can expose sensitive operator evidence

- Severity: High
- Affected boundary: conversation-scoped member-readable narrative artifact REST API and provider/admin-authored published artifact title/content text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/conversations.py` serves `GET /worlds/{world_id}/conversations/{conversation_id}/narrative` through `get_world_member_context`. Non-admin responses are restricted to published reader-visible artifacts and redact `source_run_id` plus metadata, but `_narrative_artifact_response(..., include_admin_fields=False)` returns artifact `title` and `content` unchanged.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, base64-like evidence, or provider refs embedded in published narrative artifact text despite source/metadata redaction.
- Intended remediation: blank sensitive-looking published narrative artifact titles/content for non-admin member responses while preserving safe artifact text, conversation linkage, kind, creation time, publication filtering, metadata redaction, and admin visibility; add focused regression coverage in the existing conversation narrative listing test.
- Status: Remediated in member conversation narrative artifact text redaction batch.
...
### F-141 Member conversation session title text can expose sensitive operator evidence

- Severity: High
- Affected boundary: member-readable conversation list/detail REST APIs and admin-authored conversation session title exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/conversations.py` serves `GET /worlds/{world_id}/conversations` and `GET /worlds/{world_id}/conversations/{conversation_id}` through `get_world_member_context`. `_session_response(..., include_admin_fields=False)` redacts objective, opening prompt, policy, writer config, memory config, and group context but returns `title` unchanged even though admin create/update accepts arbitrary session titles.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, base64-like evidence, or provider refs embedded in conversation session titles despite orchestration-field redaction.
- Intended remediation: blank sensitive-looking conversation session titles for non-admin member responses while preserving safe titles, identifiers, scope, mode, status, worldline/scene refs, turn progress, and admin visibility; add focused regression coverage in the existing conversation access test.
- Status: Remediated in member conversation session title text redaction batch.
...
### F-142 Player privacy export display names can expose sensitive operator evidence

- Severity: High
- Affected boundary: player privacy export REST API and player/admin-authored user/player actor display name exposure.
- Evidence: Read-only audit showed `backend/packages/player_privacy/src/noveland/player_privacy/service.py` sanitizes JSON profile data, choices, journal, notifications, interventions, and conversation reference titles, but `PlayerPrivacyProfile.display_name` and `PlayerPrivacyActorExport.display_name` are returned directly from `User.display_name` and `PlayerActorProfile.display_name`.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, base64-like evidence, or provider refs embedded in privacy export display names despite export-wide leak guards.
- Intended remediation: redact sensitive-looking profile and player actor display names in privacy exports while preserving safe names, player-owned scoping, counts, existing field omissions, request audit safety, and admin review behavior; add focused regression coverage in the existing player privacy export test.
- Status: Remediated in player privacy export display-name redaction batch.
...
### F-143 Reader media descriptors can expose sensitive asset title or description text

- Severity: High
- Affected boundary: reader/member-readable reader media list/detail REST APIs and admin/provider-authored media asset title/description exposure.
- Evidence: Read-only audit showed `backend/packages/reader_delivery/src/noveland/reader_delivery/service.py` builds `ReaderMediaDescriptor` from deliverable `MediaAsset` records. Descriptor objects omit storage URIs and use scoped download URLs, but `_descriptor_for_asset()` returns `asset.title` and `asset.description` unchanged.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, base64-like evidence, or provider refs embedded in reader media descriptor title/description text despite object storage redaction.
- Intended remediation: blank sensitive-looking reader media descriptor titles/descriptions while preserving safe asset text, scoped download URLs, object/reference descriptors, moderation filtering, and admin media management behavior; add focused regression coverage in the existing reader media list/detail/download test.
- Status: Remediated in reader media descriptor text redaction batch.
...
### F-144 Member world narrative artifact text can expose sensitive operator evidence

- Severity: High
- Affected boundary: world-level member-readable narrative artifact list/detail REST APIs and provider/admin-authored published artifact title/content text exposure.
- Evidence: Read-only audit showed `backend/services/api/src/noveland/services/api/worlds.py` serves `GET /worlds/{world_id}/narrative-artifacts` and `GET /worlds/{world_id}/narrative-artifacts/{artifact_id}` through `get_world_member_context`. Non-admin responses are restricted to published reader-visible artifacts and redact source run refs, artifact metadata, continuity metadata/status, publication metadata, publication gate evidence, source draft refs, and published-by user refs, but `_narrative_artifact_response(..., include_admin_fields=False)` returns artifact `title` and `content` unchanged.
- Impact: ordinary world members can receive raw prompt/output markers, storage refs, filesystem paths, secret/auth refs, bytes, base64-like evidence, or provider refs embedded in published narrative artifact text despite source/metadata/publication redaction.
- Intended remediation: blank sensitive-looking published narrative artifact titles/content for non-admin world-level member responses while preserving safe artifact text, publication filtering, conversation linkage, kind, timing fields, metadata redaction, and admin visibility; add focused regression coverage in the existing world narrative artifact API test.
- Status: Remediated in member world narrative artifact text redaction batch.
...
### F-145 Web error sanitizer misses storage/path variants

- Severity: High
- Affected boundary: Web client/server/proxy error normalization for backend JSON error details and browser-visible same-origin API proxy responses.
- Evidence: Read-only audit showed `web/lib/safe-error-detail.ts` recognizes `media://`, Bearer tokens, base64-like strings, and a small subset of local paths, but does not recognize `file://`, `s3://`, `gs://`, or absolute server paths such as `/root/...` even though active architecture-contracts require storage refs, filesystem/object-storage paths, and local model paths to be suppressed from Web error details.
- Impact: backend non-2xx JSON errors containing object-storage refs or server filesystem paths can be relayed through Web clients, server loaders, API proxies, or event-stream proxy setup failures into browser-visible error bodies or UI notices instead of being normalized to route-appropriate generic failure text.
- Intended remediation: extend the shared Web sensitive-error detector to cover `file://`, object-storage URL schemes, and common absolute server paths while preserving safe business validation errors; add focused proxy/client regression coverage.
- Status: Remediated in Web error storage/path variant redaction batch.
- Verification: `cd web && npm run test -- lib/auth/proxy.test.ts -t "filesystem and object-storage"` first failed because the proxy JSON error body still contained `file:///root/...` and `s3://...`, then passed after expanding shared Web sensitive-error detection; related Web proxy/client/server-loader suite `cd web && npm run test -- lib/auth/proxy.test.ts lib/realtime/proxy.test.ts lib/auth/client.test.ts lib/admin/api-client.test.ts lib/worlds/client.test.ts lib/worlds/server.test.ts lib/worlds/media.test.ts lib/beta-feedback/client.test.ts lib/beta-feedback/server.test.ts lib/private-beta/client.test.ts lib/private-beta/proxy.test.ts lib/worlds/proxy.test.ts lib/runtime/proxy.test.ts lib/api-proxy.test.ts` passed with 14 files and 87 tests; full Web `npm run lint`, `npm run typecheck`, `npm run test` with 53 files and 213 tests, `npm run build`, and `npm run check:next-env` passed; final OpenSpec strict validations and `git diff --check` passed.
