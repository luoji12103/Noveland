...
- Capture accepted planning decisions before implementation starts.
...
- `provider-backed-gm-proposal`: Use provider-kernel text generation to generate GM proposals without directly mutating world state.
...
- `narrative-quality-dashboard-api`: Expose API-first quality metrics, blockers, and repair recommendations to admins; Web dashboard work is deferred.
...
- Future backend, docs, and test work will be driven by this change's `phase-plan.md`, `tasks.md`, and capability specs. Web dashboard work requires a later approved phase.
