# Provider-backed GM Proposal Specification

## Purpose

This spec captures the current v0.6 admin-only provider-backed GM proposal capability on `main`. It covers provider-kernel text generation, invocation ledger linkage, safe prompt snapshot handling, impact classification, and review/apply boundaries.

## Requirements
### Requirement: Provider-backed GM Proposal provides the current workflow
...
- **Then** the system SHALL support the implemented GM proposal scope
...
- Legacy provider profile execution for new v0.6 generation paths.
