# MVP Provider Settings & Model Lab Specification

## Purpose
This spec captures the current v0.9 provider settings and model lab behavior on `main`. It covers configurable provider templates, server-side model discovery with manual fallback, image capability metadata, safe smoke checks, and provider-kernel secret boundaries.
