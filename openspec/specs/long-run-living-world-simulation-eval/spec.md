# Long-run Living World Simulation Eval Specification

## Purpose

This spec captures the current v0.6 long-run living world simulation eval capability on `main`. It covers drift metrics, failure reports, and reuse of `LongRunEvalRun`, multimodal evals, runtime diagnostics, and safe evidence references.

## Requirements
### Requirement: Long-run Living World Simulation Eval provides the current workflow
...
- **Then** the system SHALL support the implemented long-run eval scope
...
- Public/member eval report exposure.
