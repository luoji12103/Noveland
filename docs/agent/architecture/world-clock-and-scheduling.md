...
- `agent_calendar_entries` stores private agent calendar items, and `world_schedule_rules` stores world-owned weekday/weekend/timetable rules resolved against world time.
...
The current baseline intentionally does not implement infinite runtime loops, external schedulers, agent schedule execution, or clock-event emission from manual HTTP controls.
