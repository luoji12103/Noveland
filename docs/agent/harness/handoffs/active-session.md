...
- Date: 2026-06-09T12:05:00+08:00
- Branch: feature/audit-and-hardening-post-v1-1-rc
- Objective: post-v1.1 release-candidate audit, hardening, tests, and records under OpenSpec.
- Status: F-001 through F-043 are remediated and targeted checks passed on this branch. F-043 is ready to commit. No push performed.

## Current Context

- Active branch: feature/audit-and-hardening-post-v1-1-rc.
- Current HEAD before F-043 batch: 2cf60ee fix(web): encode scene graph api path segments.
...
- Continued the Web/e2e security audit after F-042.
- Recorded F-043: browser-side Web organization, membership, faction track, agent relationship, agent presence, agent calendar, schedule rule, and calendar conflict helpers embedded decoded world and nested identifiers directly into same-origin API paths.
...
- Encoded path segments in the scoped helper group in `web/lib/worlds/client.ts`: organizations, memberships, faction tracks, agent list/relationships/presence/calendar, schedule rules, and calendar conflicts.
...
- `cd web && npm run test -- lib/worlds/client.test.ts`: 28 passed.
- `cd web && npm run typecheck`: passed.
- `cd web && npm run lint`: passed.
- `cd web && npm run test`: 45 files and 151 tests passed. Existing React act warnings appeared in runtime-admin test output, but the suite passed.
...
- Browser-side Web organization/agent/calendar/schedule API URL construction appended decoded world and nested identifiers directly to same-origin API paths and filters.
