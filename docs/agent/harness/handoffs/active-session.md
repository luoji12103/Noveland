...
- Date: 2026-06-12T17:45:00+08:00
- Branch: feature/audit-and-hardening-post-v1-1-rc
- Objective: post-v1.1 release-candidate audit, hardening, tests, and records under OpenSpec.
- Status: F-001 through F-052 are remediated on this branch.

## Current Context

- Active branch: feature/audit-and-hardening-post-v1-1-rc.
- Current HEAD before F-052 batch: b32d998 fix(web): encode private beta feedback paths.
...
- Continued the Web/e2e security audit on server-rendered Web loaders after F-051.
- Recorded and remediated F-052: Web server workspace loaders embedded decoded route parameters and backend record identifiers directly into backend API paths.
- Added an architecture-contracts OpenSpec delta requiring Web server workspace loaders to preserve backend route templates with encoded dynamic segments and encoded query filters.
- Routed world-scoped server loader backend paths through `serverWorldPath(worldId)` and nested identifiers through `pathSegment()` in `web/lib/worlds/server.ts`, covering dashboard/workspace, agent detail, conversation list/detail/playback, player interaction/privacy, worldline comparison, narrative workspace/reader, and memory backend admin loaders.
- Expanded `web/lib/worlds/server.test.ts` with reserved-character coverage for workspace, agent, conversation, reader, worldline, and memory backend paths.
- Restored `web/next-env.d.ts` after Playwright/Next dev regenerated it to `.next/dev/types/routes.d.ts`.

## Verification This Batch

- `cd web && npm run test -- lib/worlds/server.test.ts`: 2 tests passed.
- `cd web && npm run typecheck`: passed.
- `cd web && npm run lint`: passed.
- `cd web && npm run test`: 47 files and 163 tests passed. Existing React act warnings appeared in runtime-admin test output, but the suite passed.
...
- Server-rendered Web workspace loaders appended decoded world, agent, conversation, artifact, worldline, backend record, and memory backend profile identifiers directly to backend API paths.
- The remediation encodes dynamic world and nested identifier path segments for the scoped loader group, while preserving query filters as query data.
