...
- `openspec/changes/audit-and-hardening-post-v1-1-rc/` — active post-v1.1 release-candidate audit and hardening change for backend security, Web/e2e security, product normal-use, and spec/history compliance; F-001 through F-149 are recorded with targeted remediations through provider diagnostic error-text redaction.
...
- `web/lib/` — approved web-side helpers only
  ...
  - `web/lib/worlds/server.ts` — server-rendered Web data loaders, including workspace, agent, conversation, player, reader, worldline, memory backend, provider/media/visual/speech/invocation, and diagnostics loaders with encoded backend world and nested record path segments; the world workspace loader also sanitizes client props before serialization
...
- 2026-06-12 F-088 updated existing private beta, player privacy, and moderation JSON sanitizers in `backend/packages/private_beta/src/noveland/private_beta/service.py`, `backend/packages/player_privacy/src/noveland/player_privacy/service.py`, and `backend/packages/moderation/src/noveland/moderation/service.py`, with focused coverage in `backend/tests/test_api_private_beta.py`, `backend/tests/test_api_player_privacy.py`, and `backend/tests/test_api_moderation.py`; no repository layout or package boundary changes were introduced.
- 2026-06-12 F-089 updated existing event payload sanitization ownership in `backend/packages/events/src/noveland/events/sanitization.py`, with focused coverage in `backend/tests/test_api_worlds.py` and `backend/tests/test_event_contracts.py`; no repository layout or package boundary changes were introduced.
- 2026-06-12 F-090 updated existing media and conversation presentation member JSON sanitizers in `backend/services/api/src/noveland/services/api/media.py` and `backend/services/api/src/noveland/services/api/conversation_presentations.py`, with focused coverage in `backend/tests/test_api_media.py` and `backend/tests/test_api_conversation_presentations.py`; no repository layout or package boundary changes were introduced.
...
- 2026-06-12 F-094 updated existing Web invocation ledger admin evidence redaction in `web/features/admin/invocation-ledger-admin.tsx`, with focused coverage in `web/features/admin/invocation-ledger-admin.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-095 updated existing Web provider integration admin JSON rendering and submit sanitization in `web/features/admin/provider-integration-admin.tsx`, with focused coverage in `web/features/admin/provider-integration-admin.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-096 updated existing Web memory backend admin JSON rendering and submit sanitization in `web/features/admin/memory-backend-admin.tsx`, with focused coverage in `web/features/admin/memory-backend-admin.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-097 updated existing Web runtime admin text rendering in `web/features/admin/runtime-admin.tsx`, with focused coverage in `web/features/admin/runtime-admin.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-098 updated existing Web world management dashboard JSON rendering and submit sanitization in `web/features/dashboard/world-management-dashboard.tsx`, with focused coverage in `web/features/dashboard/world-management-dashboard.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-099 updated existing Web agent builder JSON rendering, submit sanitization, and run summary/diagnostic text handling in `web/features/agents/agent-builder.tsx`, with focused coverage in `web/features/agents/agent-builder.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-100 updated existing Web preset admin JSON rendering and submit sanitization in `web/features/admin/preset-admin.tsx`, with focused coverage in `web/features/admin/preset-admin.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-101 updated existing Web world overview JSON rendering and submit sanitization in `web/features/worlds/world-overview.tsx`, with focused coverage in `web/features/worlds/world-overview.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-102 updated existing Web conversation writer config and narrative reader metadata rendering in `web/features/conversations/conversation-detail.tsx` and `web/features/worlds/narrative-reader.tsx`, with focused coverage in `web/features/conversations/conversation-detail.test.tsx` and `web/features/worlds/narrative-reader.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-103 updated existing Web provider profile admin JSON rendering and submit sanitization in `web/features/admin/provider-admin.tsx`, with focused coverage in `web/features/admin/provider-admin.test.tsx`; no package boundary changes were introduced.
- 2026-06-12 F-104 updated existing Web visual admin client CSRF handling in `web/lib/worlds/visual.ts`, with focused coverage in `web/lib/worlds/visual.test.ts`; no package boundary changes were introduced.
- 2026-06-12 F-105 updated existing Web world client CSRF handling for agent memory search in `web/lib/worlds/client.ts`, with focused coverage in `web/lib/worlds/client.test.ts`; no package boundary changes were introduced.
- 2026-06-12 F-106 introduced shared Web backend-error detail normalization in `web/lib/safe-error-detail.ts` and applied it to existing admin/world/media/private-beta/beta-feedback clients with focused coverage in `web/lib/admin/api-client.test.ts`, `web/lib/worlds/client.test.ts`, `web/lib/worlds/media.test.ts`, `web/lib/private-beta/client.test.ts`, and `web/lib/beta-feedback/client.test.ts`; Web client notices now fall back to route-specific generic errors for sensitive-looking backend details while preserving safe business errors.
- 2026-06-12 F-107 kept existing Web auth client ownership in `web/lib/auth/client.ts`, with focused coverage in `web/lib/auth/client.test.ts`; auth, CSRF, and current-subject response parsing now uses shared backend-error detail normalization with route-specific fallback messages so sensitive-looking auth backend details do not become UI-visible `AuthClientError` messages.
- 2026-06-12 F-108 kept existing Web server-loader ownership in `web/lib/worlds/server.ts` and `web/lib/beta-feedback/server.ts`, with focused coverage in `web/lib/worlds/server.test.ts` and `web/lib/beta-feedback/server.test.ts`; rethrown worlds and beta-feedback server errors now normalize sensitive-looking backend details with route-specific generic messages while fixed page load-error behavior remains unchanged.
- 2026-06-12 F-109 kept existing Web proxy response ownership in `web/lib/auth/proxy.ts`, with focused coverage in `web/lib/auth/proxy.test.ts`; same-origin Web proxies now sanitize sensitive-looking non-2xx JSON error bodies centrally before relaying network responses while preserving successful JSON, binary/no-content bodies, streaming, and auth `Set-Cookie` opt-in semantics.
- 2026-06-12 F-110 kept existing backend player-record route ownership in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; platform admins without direct world membership now share world-admin cross-user/list/create semantics for player journal, notification, and intervention management while ordinary members remain scoped to their own member-safe records.
...
- 2026-06-13 F-121 kept existing Web dashboard ownership in `web/features/dashboard/world-management-dashboard.tsx`, with focused coverage in `web/features/dashboard/world-management-dashboard.test.tsx`; dashboard runtime errors, diagnostic rows, and provider last-test messages now reuse dashboard sensitive-text redaction before rendering.
- 2026-06-13 F-122 kept existing Web proxy response ownership in `web/lib/auth/proxy.ts`, with focused coverage in `web/lib/auth/proxy.test.ts`; non-2xx proxy responses with `application/*+json` content types now receive the same sensitive JSON body redaction as `application/json`.
- 2026-06-13 F-123 kept existing Web realtime proxy ownership in `web/lib/realtime/proxy.ts`, with focused coverage in `web/lib/realtime/proxy.test.ts`; non-2xx stream setup errors now use sanitized proxy responses while successful `text/event-stream` responses remain streamed.
- 2026-06-13 F-124 kept existing backend realtime API ownership in `backend/services/api/src/noveland/services/api/realtime.py`, with focused coverage in `backend/tests/test_api_realtime.py`; conversation live WebSocket origin checks now enforce HTTP/WebSocket-equivalent scheme, hostname, and effective port before cookie-authenticated live command handling.
- 2026-06-13 F-125 kept existing backend realtime API ownership in `backend/services/api/src/noveland/services/api/realtime.py`, with focused coverage in `backend/tests/test_api_realtime.py`; member realtime conversation turn payloads now reuse sensitive-looking transcript redaction for stream deltas and live WebSocket snapshots.
- 2026-06-13 F-126 kept existing Web conversation detail ownership in `web/features/conversations/conversation-detail.tsx`, with focused coverage in `web/features/conversations/conversation-detail.test.tsx`; live WebSocket command errors now use shared sensitive backend-detail normalization before becoming user-visible notices.
...
- 2026-06-13 F-131 updated existing backend reader media delivery in `backend/packages/reader_delivery/src/noveland/reader_delivery/service.py`, with focused coverage in `backend/tests/test_api_reader_media.py`; no new package, route, storage, or provider boundary was introduced.
- 2026-06-13 F-132 updated existing backend moderation target validation in `backend/packages/moderation/src/noveland/moderation/service.py`, with focused coverage in `backend/tests/test_api_moderation.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-133 updated existing backend worlds API calendar response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-134 updated existing backend worlds API scene/location response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-135 updated existing backend worlds API organization/faction response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-136 updated existing backend worlds API worldline/agent response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-137 updated existing backend worlds API schedule rule response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-138 updated existing backend worlds API player actor response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-139 updated existing backend worlds API world response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-140 updated existing backend conversation API narrative artifact response shaping in `backend/services/api/src/noveland/services/api/conversations.py`, with focused coverage in `backend/tests/test_api_conversations.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-141 updated existing backend conversation API session response shaping in `backend/services/api/src/noveland/services/api/conversations.py`, with focused coverage in `backend/tests/test_api_conversations.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-142 updated existing backend player privacy export response shaping in `backend/packages/player_privacy/src/noveland/player_privacy/service.py`, with focused coverage in `backend/tests/test_api_player_privacy.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-143 updated existing backend reader delivery descriptor response shaping in `backend/packages/reader_delivery/src/noveland/reader_delivery/service.py`, with focused coverage in `backend/tests/test_api_reader_media.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-144 updated existing backend worlds API narrative artifact response shaping in `backend/services/api/src/noveland/services/api/worlds.py`, with focused coverage in `backend/tests/test_api_worlds.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-145 kept existing shared Web backend-error detail normalization ownership in `web/lib/safe-error-detail.ts` and existing proxy response coverage in `web/lib/auth/proxy.test.ts`; Web client/proxy/server-loader error sanitization now treats `file://`, `s3://`, `gs://`, and common absolute server paths as sensitive while preserving safe business errors.
- 2026-06-13 F-146 updated existing provider execution visibility ownership in `backend/packages/providers/src/noveland/providers/contracts.py`, `backend/packages/providers/src/noveland/providers/registry.py`, `backend/packages/providers/src/noveland/providers/service.py`, and `backend/services/api/src/noveland/services/api/providers.py`, with focused coverage in `backend/tests/test_api_providers.py` and `backend/tests/test_provider_execution_service.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-147 updated existing media/speech/authoring/narrative-quality provider execution visibility ownership in `backend/packages/media/src/noveland/media/image_service.py`, `backend/packages/speech/src/noveland/speech/service.py`, `backend/packages/authoring/src/noveland/authoring/service.py`, `backend/packages/narrative_quality/src/noveland/narrative_quality/service.py`, and their API route callers, with focused coverage in `backend/tests/test_api_images.py` and `backend/tests/test_api_speech.py`; no new route, package, storage, or provider boundary was introduced.
- 2026-06-13 F-148 kept existing Web worlds server-loader ownership in `web/lib/worlds/server.ts`, with focused coverage in `web/lib/worlds/server.test.ts`; world workspace loader results now omit forbidden keys and redact sensitive-looking string values before data crosses into `WorldOverview` client props while preserving safe sibling fields and existing client display/submit sanitizers.
- 2026-06-13 F-149 kept existing provider secret, health, and execution ownership in `backend/packages/providers/src/noveland/providers/secrets.py`, `backend/packages/providers/src/noveland/providers/health.py`, and `backend/packages/providers/src/noveland/providers/service.py`, with focused coverage in `backend/tests/test_provider_registry_service.py` and `backend/tests/test_provider_execution_service.py`; provider health and execution failure diagnostics now redact sensitive-looking error text before health-check, invocation, prompt-snapshot, or media-job evidence persists while preserving safe business errors.
